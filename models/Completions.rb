class Completions
  ...
  def completions_of_level(level_id)
  ...
  def completions_by_user(user_id)
    return db.execute("SELECT COUNT(user_id) FROM completions WHERE AND user_id = ? AND percentage = 100", [level_id.to_i, user_id.to_i]).first['COUNT(user_id)'].to_i
  end
